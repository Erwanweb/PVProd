...
class BasePlugin:
    ...
    def onStart(self):
        ...
        if 6 not in Devices:
            Domoticz.Device(Name="Autocons", Unit=6, Type=243, Subtype=31, Used=1).Create()
            devicecreated.append(deviceparam(6, 0, "0"))  # default is 0 Kwh forecast
        if 7 not in Devices:
            Domoticz.Device(Name="Co2", Unit=7, Type=243, Subtype=31, Used=1).Create()
    ...
    # CALC CO & %  ------------------------------------------------------------------------------------------------
        ...
        self.AutoCons = (self.TodayPVCons / self.TodayPVProd)*100)
        self.C02 = ((self.TodayPVProd/1000)*0.06)
