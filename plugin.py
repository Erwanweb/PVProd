...
class BasePlugin:
    ...
    def __init__(self):
        ...
        self.lastTimeReset = datetime.now()
    ...
    def onStart(self):
        ...
        Domoticz.Status("Variables are = {}, {}, {}, {}, {}".format(self.TodayEnergyImport, self.TodayPVInject, self.TodayPVCons, self.TodayPVAutoCons, self.lastTimeReset))
    ...
    def onHeartbeat(self):
        ...
        #if now > self.lastTimeReset - timedelta(days=1)
            self.lastTimeReset = now
            self.TodayPVAutoCons = 0
            self.WriteLog("Reset Today Counter value", "Verbose")
            Domoticz.Status("Reset Today Counter value")
    ...
    def getUserVar(self):
        ...
        if variables:
            ...
            if novar:
                ...
                Domoticz.Debug("UserVariable Created !!!")
            else:
                try:
                    self.Internals.update(eval(valuestring))
                    Domoticz.Debug("UserVariable loaded")
                except:
                    self.Internals = self.InternalsDefaults.copy()
                    Domoticz.Debug("UserVariable reseted")
                return
        else:
