...
# Version:    0.0.1: alpha...
# Version:    0.0.2: beta
# Version:    0.0.3: forecast.solar 'key' field, json path fixed, distutils removed


"""
<plugin key="RL-SOLARPV" name="Ronelabs - Solar PV Control plugin" author="Ronelabs" version="0.0.3" externallink="https://ronelabs.com">
    <description>
...
class BasePlugin:
    ...
    def onStart(self):
        ...
        #Forecast devices
        if 9 not in Devices:
            Domoticz.Device(Name="FC Today", Unit=9, Type=243, Subtype=31, Options={"Custom": "1;Kwh"}, Used=1).Create()
            devicecreated.append(deviceparam(9, 0, "0"))  # default is 0 Kwh forecast
        if 10 not in Devices:
            Domoticz.Device(Name="FC Tomorrow", Unit=10, Type=243, Subtype=31, Options={"Custom": "1;Kwh"}, Used=1).Create()
    ...
    # HEARTBEAT FORECAST PART  ---------------------------------------------------
        ...
        if self.ForecastRequest <= now:
            ...
            if latlon:
                ...
                if (not SFDatas) or (not isinstance(SFDatas, dict)):
                    ...
                    forecast = SFDatas.get('forecast', {})
                    summary = forecast.get('summary-wh-day', {})
                    hourly = forecast.get('hourly-watts', {})
                    # Total wh forecast
                    self.J0TotalValue = round(float(summary.get('today', 0)) / 1000, 3)
                    self.J1TotalValue = round(float(summary.get('tomorrow', 0)) / 1000, 3)
                    # Watts per hour in Raw : one value per hour, 0 for the hours the API did not return
                    hourlytoday = hourly.get('today', {})
                    hourlytomorrow = hourly.get('tomorrow', {})
                    self.J0WperHRaw = ",".join(str(hourlytoday.get(str(h), 0)) for h in range(24))
                    self.J1WperHRaw = ",".join(str(hourlytomorrow.get(str(h), 0)) for h in range(24))
        ...
        if variables:
            ...
            if novar:
                ...
                if domoticzInfo is None:
                    ...
                    if domoticzInfo and VersionTuple(domoticzInfo.get("dzvents_version", "0")) >= (2, 4, 9):
...
def SolarForecatAPI(APICall):
    ...
    if PluginFolder == "":
        ...
        jsonFile = "/home/domoticz/plugins/PVProd/solar_forecast.json"
    else:
...
def VersionTuple(version):
    # replaces distutils.LooseVersion, removed from Python 3.12
