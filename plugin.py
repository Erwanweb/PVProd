...
class BasePlugin:
    ...
    def onHeartbeat(self):
        ...
        # update E MEters
        self.readCons()
        Domoticz.Debug("EnergyImport value is = {}".format(self.EnergyCons))
        self.readPVProd()
        Domoticz.Debug("EnergyImport value is = {}".format(self.self.PVProd))

        # Make the calc for other devices
        self.EnergyImport = (self.EnergyCons - self.PVProd)
        strValue = str(self.EnergyImport) + ";12345"
        Devices[2].Update(nValue=0, sValue=strValue)  # update the dummy device showing the current value
        Domoticz.Debug("EnergyImport value is = {}".format(self.EnergyImport))

        self.PVInject = (self.PVProd - self.EnergyCons)
        if self.PVInject <= 0:
            self.PVInject = 0
            strValue = str(self.PVInject) + ";12345"
            Devices[5].Update(nValue=0, sValue=strValue)  # update the dummy device showing the current value
            Domoticz.Debug("PVInject value is = {}".format(self.PVInject))
            self.PVCons = self.PVProd
            strValue = str(self.PVCons) + ";12345"
            Devices[4].Update(nValue=0, sValue=strValue)  # update the dummy device showing the current value
            Domoticz.Debug("PVCons value is = {}".format(self.PVCons))
        else :
            strValue = str(self.PVInject) + ";12345"
            Devices[5].Update(nValue=0, sValue=strValue)  # update the dummy device showing the current value
            Domoticz.Debug("PVInject value is = {}".format(self.PVInject))
    ...
    def readCons(self):
            ...
            for device in devicesAPI["result"]:  # parse the devices for temperature sensors
                ...
                if idx in self.EnergyConsMeter:
                    if "Usage" in device:
                        ...
                        Domoticz.Debug(f"E Meter General cons value: {valeur}")
                        #return int(valeur)
        ...
        if nbtemps > 0:
            self.EnergyCons = round(sum(listintemps))
            strValue = str(self.EnergyCons) + ";" + str(self.EnergyCons)
        ...
        self.WriteLog("E Meter General cons calculated value is = {}".format(self.EnergyCons), "Verbose")
        return noerror
    ...
    def readPVProd(self):
            ...
            for device in devicesAPI["result"]:  # parse the devices for temperature sensors
                ...
                if idx in self.PVProdMeter:
                    if "Usage" in device:
                        ...
                        texte = (device["Usage"])
                        valeur = texte.replace("Watt", "").strip()
                        Domoticz.Debug(f"E Meter PVProd value: {valeur}")
                        #return int(valeur)
                        listintemps.append(int(valeur))
                    else:
                        Domoticz.Error("device: {}-{} is not a E Meter sensor".format(device["idx"], device["Name"]))
        ...
        nbtemps = len(listintemps)
        if nbtemps > 0:
            self.PVProd = round(sum(listintemps))
            strValue = str(self.PVProd) + ";" + str(self.PVProd)
            Devices[3].Update(nValue=0, sValue=strValue, TimedOut = 0)  # update the dummy device showing the current value
        else:
            Domoticz.Debug("No E Meter PVProd found... ")
            noerror = False

        self.WriteLog("E Meter PVProd calculated value is = {}".format(self.PVProd), "Verbose")
        return noerror
