...
class BasePlugin:
    ...
    def onStart(self):
        ...
        devicecreated = []
        if 1 not in Devices:
            Domoticz.Device(Name="General Cons", Unit=1, Type=243, Subtype=29, Used=1).Create()
    ...
    def onHeartbeat(self):
        ...
        if now.date() > LastReset.date():
            ...
            self.C02 = 0
            self.AutoCons = 0
            Domoticz.Debug("Reset Today Counter value")
        ...
        if self.EnergyImport <= 0:
            self.EnergyImport = 0
        self.TodayEnergyImport = self.TodayEnergyImport + (self.EnergyImport * hours)
        ...
        if self.PVInject <= 0:
            ...
            self.TodayPVCons = self.TodayPVCons + (self.PVCons * hours)
            strValue4 = str(self.PVCons) + ";" + str(self.TodayPVCons)
        else :
            self.PVCons = (self.PVProd - self.PVInject)
            self.TodayPVCons = self.TodayPVCons + (self.PVCons * hours)
            strValue4 = str(self.PVCons) + ";" + str(self.TodayPVCons)
        self.TodayPVInject = self.TodayPVInject + (self.PVInject * hours)
        strValue5 = str(self.PVInject) + ";" + str(self.self.TodayPVInject)

    # CALC CO & %  ------------------------------------------------------------------------------------------------

        self.AutoCons = round(((self.TodayPVCons / self.TodayPVProd)*100))
        self.C02 = round((0-((self.TodayPVProd/1000)*0.06)), 2)
    ...
    # UPDATING DEVICES ------------------------------------------------------------------------------------------------
        ...
        Domoticz.Debug("Free Co2 value is = {}".format(str(self.C02)))
    ...
    def readCons(self):
            ...
            for device in devicesAPI["result"]:  # parse the devices for temperature sensors
                ...
                if idx in self.EnergyConsMeter:
                    if "Usage" in device:
                        ...
                        texte2 = (device["CounterToday"])
                        valeur2 = texte2.replace("kWh", "").strip()
                        #valeur2inW = (float(valeur2)*1000)
        ...
        self.WriteLog("E Meter General cons calculated value is = {}w, and {}wh".format(self.EnergyCons,self.TodayEnergyCons), "Verbose")
        return noerror
    ...
    def readPVProd(self):
        ...
        self.WriteLog("E Meter PVProd calculated value is = {}w, and {}wh".format(self.PVProd, self.TodayPVProd), "Verbose")
        return noerror
