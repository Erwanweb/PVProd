...
def entrytimestamp(entry):
    # forecast.solar renamed the 'timestamp' field to 'key' in the key-value format,
    # accept both so the script keeps working whichever one the API returns
    return entry.get("key", entry.get("timestamp"))
...
# Check forecast for total Wh per day ----------------------------------------------------------------------------------
    if 'result' in data and 'watt_hours_day' in data['result']:  # Power per hour part
        ...
        for entry in data['result']['watt_hours_day']:
            timestamp = datetime.strptime(entrytimestamp(entry), "%Y-%m-%d")
...
# Check forecast for watt per hour -------------------------------------------------------------------------------------
    if 'result' in data and 'watts' in data['result']:  # Power per hour part watt_hours_day
        ...
        for entry in data['result']['watts']:
            timestamp = datetime.strptime(entrytimestamp(entry), "%Y-%m-%d %H:%M:%S")
            hourly_watts = entry['value']
            # Creating variables
            now = datetime.now()
            Today = now.strftime("%d")
